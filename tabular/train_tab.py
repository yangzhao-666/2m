...
from RB import TwoMBuffer, ReplayBuffer
...
def evaluate(TwoM, eval_env):
    ...
    for i in range(20):
        eps_returns = 0
        s = eval_env.reset()
        s = eval_env.dim1to2(s)
        done = False
        while not done:
            action = agent.select_action(s)
            n_s, r, done, info = eval_env.step(action)
            n_s = eval_env.dim1to2(n_s)
            s = n_s
            eps_returns += r
            trajs[s] += 1
...
def train(TwoM, env, eval_env, config, wandb_session):
    ...
    i_steps = 0
    i_episode = 0
    train_on_states = np.zeros((7, 10))
    collected_data = {'ec': np.zeros((7, 10)), 'rl': np.zeros((7, 10))}
    while i_steps < config.total_steps:
        ...
        if i_episode % config.eval_freq == 0:
            ...
            data_ec_p = sum(TwoM.rb.rb_where) / TwoM.rb.cntr
            wandb_session.log({'eval return': eval_return, 'rl score': rl_score, 'ec score': ec_score, 'training steps': i_steps, 'eps': epsilon, 'ec factor': ec_factor, 'mixed data ec percentage': data_ec_p, 'rb size': TwoM.rb.cntr})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ec_start', type=float, default=0.9)
    parser.add_argument('--ec_end', type=float, default=0.01)
    parser.add_argument('--ec_decay_steps', type=int, default=20000)
    ...
    for run in range(args.runs):
        ...
        rb = ReplayBuffer(capacity=config.rb_capacity, batch_size=32)
