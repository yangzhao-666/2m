...
class MFECAgent():
    def __init__(self, buffer_size, k, discount, n_actions, config):
        self.memory = []
...
class ActionBuffer():
    ...
    def find_states(self, state):
        if self._tree:
            neighbor_idx = self._tree.query(np.array(state).reshape(1, -1))[1][0][0]
...
class QEC(): #q values for episodic controller;
    ...
    def estimate(self, state, action):
        ...
        if len(buffer) <= self.k:
            #print('len of buffer smaller than k')
            return float('inf')
