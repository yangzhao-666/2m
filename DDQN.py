...
class DDQNAgent():
    ...
    def update(self, batch_data):
        ...
        n_a = torch.argmax(self.Q_net(batch_n_s), dim=1)
        td_target = self.gamma * (1 - batch_done).unsqueeze(1) * self.Q_target_net(batch_n_s).gather(1, n_a.unsqueeze(1)) + batch_r.unsqueeze(1)
        current_q = self.Q_net(batch_s).gather(1, torch.LongTensor(batch_a).unsqueeze(1))
        td_error = self.criterion(td_target.squeeze(1), current_q.squeeze(1))
...
class QConvNet(nn.Module):
    def __init__(self, width, height, in_ch, n_actions):
        super().__init__()
        self.conv1 = nn.Conv2d(in_ch, 16, kernel_size=3, stride=2)
    ...
    def forward(self, x):
        # NHWC -> NCHW
        x = x.permute(0, 3, 1, 2) / 255 # [0, 255] -> [0, 1]
