...
if __name__ == '__main__':
    ...
    parser.add_argument('--rb_capacity', type=int, default=250000)
    parser.add_argument('--rl_alg', type=str, default='DQN')
    parser.add_argument('--mfec_buffer_size', type=int, default=125000)
    ...
    for run in range(args.runs):
        ...
        '''
        env = gym.make(config.env_name, sticky_action_prob=0)
        eval_env = gym.make(config.env_name, sticky_action_prob=0)
        ...
        ec = MFECAgent(buffer_size=config.mfec_buffer_size, n_actions=n_actions, config=config, discount=config.gamma)
